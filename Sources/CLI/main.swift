...
do {
    ...
    case COMMANDS[1]:
        ...
        if isBase64data {
            try encrypt(password: &password, plaintext: &data,ver: .X).base64EncodedData().write(to: outURL)
        } else {
            try encrypt(password: &password, plaintext: &data,ver: .X).write(to: outURL)
        }
